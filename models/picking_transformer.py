"""
    Module for Asset Picking Transformer
...
from models.cfg.picking_transformer_config\
    import PICKING_TRANSFORMER_CONFIG


class PickingTransformer(nn.Module):
    """
        Picking Transformer
    """
    def __init__(self, config: dict = None):
        ...
        if config is None:
            config = PICKING_TRANSFORMER_CONFIG
        self.config = config
    ...
    def forward(self, obs_in, softmax=True,
        ...
        outputs = self.dt(fusion_embeds, seq_mask=seq_mask,
                        src_key_padding_mask=src_key_padding_mask)

        preds = self.allocator(outputs).view(batch_size, asset_num)
