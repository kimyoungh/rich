"""
    Module for Trading BC Transformer
...
from models.cfg.trading_bc_transformer_config\
    import TRADING_BC_TRANSFORMER_CONFIG


class TradingBCTransformer(nn.Module):
    """
    ...
    def __init__(self, config: dict = None):
        ...
        if config is None:
            config = TRADING_BC_TRANSFORMER_CONFIG
        self.config = config
    ...
    def forward(self, assets_in, obs_in, actions_in,
        ...
        assert actions_in.shape[1] == rewards_in.shape[1] == seq_len
        ...
        reward_preds = self.reward_preds(outputs[:, 1]).squeeze(-1)

        return action_preds, reward_preds, outputs
