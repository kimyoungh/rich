"""
    Training procedure for Trading BC Transformer
...
from models.cfg.trading_bc_transformer_config import (
    TRADING_BC_TRANSFORMER_TRAIN_CONFIG)
from datasets.trading_bc_transformer_dataset import TradingBCTransformerDataset
...
def train_trader(config):
    ...
    model = TradingBCTransformer(config).to(device)
    model.eval()
...
def get_data_loaders(datasets_path, asset_idx=0,
    ...
    train_dataset = TradingBCTransformerDataset(
        observations[train_indices], action_series[train_indices],
        rew_series[train_indices], asset_idx)
    valid_dataset = TradingBCTransformerDataset(
        observations[valid_indices], action_series[valid_indices],
        rew_series[valid_indices], asset_idx)
...
def train(model, optimizer, train_loader, device=None):
    ...
    for epoch, (assets_in, obs,
                actions, rewards) in enumerate(train_loader):
        assets_in = assets_in.to(device)
        obs = obs.to(device)
        actions = actions.to(device)
        rewards = rewards.to(device)

        a_preds, _, _ = model(assets_in, obs, actions, rewards)
...
def validation(model, valid_loader, device=None):
    ...
    for batch, (assets_in, obs,
        actions, rewards) in enumerate(valid_loader):
        assets_in = assets_in.to(device)
        obs = obs.to(device)
        actions = actions.to(device)
        rewards = rewards.to(device)
...
def _parser():
    ...
    parser.add_argument(
        "--model_name", type=str, default="trading_bc_transformer",
        help="model name")
    parser.add_argument(
...
def main():
    ...
    config = TRADING_BC_TRANSFORMER_TRAIN_CONFIG
